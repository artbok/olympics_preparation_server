...
def start_new_round(duelName):
    ...
    # Check if game over (Indices 0, 1, 2)
    if duel.index >= 3:
        finish_game(duelName)
        return
        
    duration = 300
    if duel.tasks[duel.index].difficulty == "Простой": duration = 60 # Adjusted for realism
...
def match_found(player1: User, player2: User):
    ...
    # Register event handler strictly for this duel
...
def handle_duel(duelName, data):
    ...
    if operation == "answer":
        ...
        # Allow 2 seconds buffer for network latency
        ...
        if (duel.player1.scores[duel.index] is not None and 
            ...
            socketio.sleep(3) # Non-blocking sleep for UI delay
...
def handle_matchmaking(username):
    ...
    # Remove existing queue entry for this user if present to prevent duplicates
    global userQueue
    ...
    for item in userQueue:
        # Match if within 200 Elo rating
